      ...
      let%lwt s =
        if%lwt cmp x1 x2 le 0 then
          if%lwt cmp x2 x3 le 0 then Lwt.return [x1; x2; x3]
            ...
            if%lwt cmp x1 x3 le 0 then Lwt.return [x1; x3; x2]
            else Lwt.return [x3; x1; x2]
        else
          if%lwt cmp x1 x3 le 0 then Lwt.return [x2; x1; x3]
          else
            if%lwt cmp x2 x3 le 0 then Lwt.return [x2; x3; x1]
      ...
      let%lwt s =
        if%lwt cmp x1 x2 gt 0 then
          if%lwt cmp x2 x3 gt 0 then Lwt.return [x1; x2; x3]
            ...
            if%lwt cmp x1 x3 gt 0 then Lwt.return [x1; x3; x2]
            else Lwt.return [x3; x1; x2]
        else
          if%lwt cmp x1 x3 gt 0 then Lwt.return [x2; x1; x3]
          else
            if%lwt cmp x2 x3 gt 0 then Lwt.return [x2; x3; x1]
            else Lwt.return [x3; x2; x1]
      in
      Lwt.return (s, tl)
    | n, l ->
