...
type (_, _, _) t =
  ...
  | Save : ((Status.t option -> TuneCore.t Entry.t -> int -> Music.key -> string -> PersonCore.t Entry.t list option -> string option -> string option -> string -> Datetime.t -> Datetime.t -> 'w), 'w, VersionCore.t Entry.t) t
...
let route : type a w r. (a, w, r) t -> (a, w, r) route = function
  ...
  | Save -> literal "make-and-save" @@ query_opt "status" (module Status) @@ query "tune" (module Entry.J(TuneCore)) @@ query "bars" (module JInt) @@ query "key" (module Music.Key) @@ query "structure" (module JString) @@ query_opt "arrangers" (module JList(Entry.J(PersonCore))) @@ query_opt "remark" (module JString) @@ query_opt "disambiguation" (module JString) @@ query "content" (module JString) @@ query "modified_at" (module Datetime) @@ query "created_at" (module Datetime) @@ return (module Entry.J(VersionCore))
