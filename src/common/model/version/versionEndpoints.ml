...
type (_, _, _) t =
  ...
  | Save : ((Status.t option -> Datetime.t -> Datetime.t -> VersionCore.t -> 'w), 'w, VersionCore.t Entry.t) t
...
let route : type a w r. (a, w, r) t -> (a, w, r) route = function
  ...
  | Save -> literal "save" @@ query_opt "status" (module Status) @@ query "modified_at" (module Datetime) @@ query "created_at" (module Datetime) @@ query "version" (module VersionCore) @@ return (module Entry.J(VersionCore))
