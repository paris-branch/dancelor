...
type (_, _, _) t =
  ...
  | MakeAndSave : ((Status.t option -> TuneCore.t -> int -> Music.key -> string -> PersonCore.t list option -> string option -> string option -> bool option -> string -> Datetime.t -> Datetime.t -> 'w), 'w, VersionCore.t) t
...
let all = [W Get; W Search; W MakeAndSave; W MarkBroken; W MarkFixed; W Ly; W Svg; W Ogg; W Pdf]
...
let route : type a w r. (a, w, r) t -> (a, w, r) route = function
  ...
  | MakeAndSave -> literal "make-and-save" @@ query_opt "status" (module Status) @@ query "tune" (module TuneCore) @@ query "bars" (module MInteger) @@ query "key" (module Music.Key) @@ query "structure" (module MString) @@ query_opt "arrangers" (module MList(PersonCore)) @@ query_opt "remark" (module MString) @@ query_opt "disambiguation" (module MString) @@ query_opt "broken" (module MBool) @@ query "content" (module MString) @@ query "modified_at" (module Datetime) @@ query "created_at" (module Datetime) @@ return (module VersionCore)
