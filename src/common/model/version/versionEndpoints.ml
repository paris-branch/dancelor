...
let route : type a w r. (a, w, r) t -> (a, w, r) route = function
  ...
  | Save -> literal "make-and-save" @@ query_opt "status" (module Status) @@ query "tune" (module TuneCore) @@ query "bars" (module JInt) @@ query "key" (module Music.Key) @@ query "structure" (module JString) @@ query_opt "arrangers" (module JList(PersonCore)) @@ query_opt "remark" (module JString) @@ query_opt "disambiguation" (module JString) @@ query_opt "broken" (module JBool) @@ query "content" (module JString) @@ query "modified_at" (module Datetime) @@ query "created_at" (module Datetime) @@ return (module VersionCore)
  | MarkBroken -> literal "mark-broken" @@ variable (module SSlug(VersionCore)) @@ return (module JUnit)
  | MarkFixed -> literal "mark-fixed" @@ variable (module SSlug(VersionCore)) @@ return (module JUnit)
