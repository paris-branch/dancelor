...
type (_, _, _) t =
  ...
  | Save : ((Status.t option -> string -> int option -> Datetime.t -> Datetime.t -> 'w), 'w, PersonCore.t Entry.t) t
...
let route : type a w r. (a, w, r) t -> (a, w, r) route = function
  ...
  | Save -> literal "make-and-save" @@ query_opt "status" (module Status) @@ query "name" (module JString) @@ query_opt "scddb-id" (module JInt) @@ query "modified-at" (module Datetime) @@ query "created-at" (module Datetime) @@ return (module Entry.J(PersonCore))
