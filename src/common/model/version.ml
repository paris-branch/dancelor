...
module Endpoint = struct
  open Madge_common
  let get = endpoint ~path:"/version" (module Self)
  let all = endpoint ~path:"/version/all" (module MList (Self))
  let search = endpoint ~path:"/version/search" (module MList (Score.Make_Serialisable (Self)))
