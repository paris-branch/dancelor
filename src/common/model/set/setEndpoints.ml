...
type (_, _, _) t =
  ...
  | MakeAndSave : ((Status.t option -> string -> PersonCore.t list option -> Kind.Dance.t -> (VersionCore.t * VersionParameters.t) list option -> SetOrder.t -> DanceCore.t list option -> Datetime.t -> Datetime.t -> 'w), 'w, SetCore.t) t
...
let all = [W Get; W Search; W MakeAndSave; W Delete; W Pdf]
