...
type (_, _, _) t =
  ...
  | Sign_in : ((string -> User.Password_clear.t -> bool -> 'w), 'w, User.entry option) t
  | Sign_out : ('w, 'w, unit) t
  | Create : ((User.t -> 'w), 'w, User.entry * User.Password_reset_token_clear.t) t
  | Prepare_reset_password : ((string -> 'w), 'w, User.Password_reset_token_clear.t) t
  | Reset_password : ((string -> User.Password_reset_token_clear.t -> User.Password_clear.t -> 'w), 'w, unit) t
...
let route : type a w r. (a, w, r) t -> (a, w, r) route =
  ...
  function
    ...
    | Sign_in -> literal "sign-in" @@ body "username" (module JString) @@ body "password" (module User.Password_clear) @@ body "remember-me" (module JBool) @@ post (module JOption(Entry.JPublic(User)))
    ...
    | Prepare_reset_password -> literal "prepare-reset-password" @@ body "username" (module JString) @@ post (module User.Password_reset_token_clear)
    | Reset_password -> literal "reset-password" @@ body "username" (module JString) @@ body "token" (module User.Password_reset_token_clear) @@ body "password" (module User.Password_clear) @@ post (module JUnit)
