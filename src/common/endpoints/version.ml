...
| BuildSnippets : ((Version.t Entry.Id.t -> VersionParameters.t -> Rendering_parameters.t -> 'w), 'w, Snippet_ids.t Job.registration_response copyright_response) t
| BuildPdf : ((Version.t Entry.Id.t -> VersionParameters.t -> Rendering_parameters.t -> 'w), 'w, Job_id.t Job.registration_response copyright_response) t
(* Files related to an anonymous version *)
| BuildSnippets' : ((Version.t -> VersionParameters.t -> Rendering_parameters.t -> 'w), 'w, Snippet_ids.t Job.registration_response) t
...
let route : type a w r. (a, w, r) t -> (a, w, r) route =
  ...
  function
    ...
    (* Files related to a version *)
    | BuildSnippets -> literal "build-snippets" @@ variable (module Entry.Id.S(Version)) @@ query "parameters" (module VersionParameters) @@ query "rendering-parameters" (module Rendering_parameters) @@ post (module Copyright_response(Job.Registration_response(Snippet_ids)))
    | BuildPdf -> literal "build-pdf" @@ variable (module Entry.Id.S(Version)) @@ query "parameters" (module VersionParameters) @@ query "rendering-parameters" (module Rendering_parameters) @@ post (module Copyright_response(Job.Registration_response(Job_id)))
    (* Files related to an anonymous version *)
    | BuildSnippets' -> literal "build-snippets" @@ query "version" (module Version) @@ query "parameters" (module VersionParameters) @@ query "rendering-parameters" (module Rendering_parameters) @@ post (module Job.Registration_response(Snippet_ids))
