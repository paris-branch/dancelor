          ...
          ~onclick: (fun () ->
            Option.fold
              ...
              ~some: (fun (username, password, remember_me) ->
                ...
                match%lwt Madge_client.call Endpoints.Api.(route @@ Auth Login) username password remember_me with
...
let logout () =
  Madge_client.call Endpoints.Api.(route @@ Auth Logout);%lwt
  Js_of_ocaml.Dom_html.window##.location##reload;
  Lwt.return_unit

let header_item =
  let status_lwt = Madge_client.call Endpoints.Api.(route @@ Auth Status) in
  L.li
    ~a: [
      L.a_class
        (
