...
= (module struct
  ...
  let initialise (initial_value, initial_params) =
    ...
    let (params, set_params) = S.create None in
    (* initially, there might not be parameters *)
    let set_params = set_params % some in
    let%lwt editor = Editor.initialise editor @@ Editor.QuickEdit (initial_params, set_params) in
    let%lwt initial_params = Editor.result editor in
    Option.iter set_params initial_params;
    lwt {comp; params; editor}

  let signal p =
    RS.bind (Component.signal p.comp) @@ fun value ->
    RS.bind (S.map (Option.to_result ~none: "You must set parameters.") p.params) @@ fun params ->
