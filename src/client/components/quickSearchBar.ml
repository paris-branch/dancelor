...
let make_and_render ?(number_of_results=10) ~placeholder ~search ~make_result ?on_enter ?autofocus () =
  ...
  let search_bar = SearchBar.make ~search ~min_characters ~slice () in
  ...
  div ~a:[a_class ["search-bar"]] [
    ...
    SearchBar.render ~placeholder ~on_focus:(fun () -> set_table_visible true) ?on_enter ?autofocus search_bar;
    ...
    tablex
      ...
      [
        R.tbody (
          S.bind_s' (SearchBar.state search_bar) [] @@ function
