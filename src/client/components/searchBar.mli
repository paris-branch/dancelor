...
module Quick : sig
  val make_and_render :
    ...
    (** Wrapper around {!make} and {!render} returning a quick search bar. This
        is a search bar with a floating table. The table reports hints or errors
        and shows the search results. The minimum number of characters is set to
        [3] and the slice picks only one page of [10] entries.
