    ...
    let sets =
      Selector.make
        ...
        ~search: (fun slice input ->
          ...
          Lwt.map Result.ok @@
            Madge_client.call Endpoints.Api.(route @@ Set Search) slice filter
        )
  ...
  let submit ~edit (editor : t) =
    ...
    | (Some {name; date; sets}, slug) ->
      Lwt.map Option.some @@
        (
          match slug with
          | None -> Madge_client.call Endpoints.Api.(route @@ Book Create)
          | Some slug -> Madge_client.call Endpoints.Api.(route @@ Book Update) slug
        )
          (
            Model.Book.make
              ~title: name
