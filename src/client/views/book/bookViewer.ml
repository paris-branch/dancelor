...
let display_warnings warnings =
  ...
  let rec display_sets = function
    | [] -> []
    | (None, n) :: tl ->
      ([text "standalone"] @ display_times n) :: display_sets tl
    | (Some set, n) :: tl ->
      ([text "in “"; span_lwt (Formatters.Set.name set); text "”"] @ display_times n) :: display_sets tl
  in
  ...
  let display_warning warning =
    ...
    | Book.DuplicateVersion (tune, sets_opt) ->
      li
        (text "Tune “" :: span_lwt (Formatters.Tune.name tune) :: text "” appears several times: " :: (display_sets sets_opt |> format_set_list))
