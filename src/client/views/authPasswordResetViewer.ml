  ...
  Page.make
    ...
    ~buttons: [
      Button.make
        ...
        ~onclick: (fun () ->
          Madge_cohttp_lwt_client.call Endpoints.Api.(route @@ Auth ResetPassword) username token (Result.get_ok @@ S.value password);%lwt
          Components.Toast.open_ ~title: "Password reset" [txt "Your password has been reset successfully. You may now try to login."];
