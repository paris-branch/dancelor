    ...
    [
      ...
      div
        ...
        (
          ...
          let b_parameters =
            VersionParameters.(make
              ~transposition: (Transposition.relative b_pitch Music.pitch_c)
              ())
          in
          let e_pitch = Music.make_pitch E Flat 0 in
          let e_parameters =
            VersionParameters.(make
              ~transposition: (Transposition.relative e_pitch Music.pitch_c)
              ())
          in
          let c_pdf_href, b_pdf_href, e_pdf_href, bass_pdf_href, ly_href =
            ...
            Helpers.build_path
              ~api: true
              ~route: (Router.VersionPdf slug)
              ~query: [
                "parameters",
                [
                  b_parameters
                  |> VersionParameters.to_yojson
        ...
        (
          ...
          let broken =
            Inputs.Button.create
              ...
              ~text: (if is_broken then "Mark fixed" else "Mark broken")
              page
          in
