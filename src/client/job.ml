...
let copyright_reponse_promise_to_copyright_reason_promise copyright_response_promise =
  match%lwt copyright_response_promise with
  | Error error -> raise (Madge_client.Error error)
  | Ok Endpoints.Version.Copyright_response.Protected -> lwt_none
  | Ok Endpoints.Version.Copyright_response.Granted {reason; _} -> lwt_some reason
