...
let endpoint ?(meth = `POST) ~path returns = {meth; path; returns}
...
module MOption
    (A : SERIALISABLE) : SERIALISABLE with
  type t = A.t option
= struct
...
module MPair
    (A : SERIALISABLE)
    (B : SERIALISABLE) : SERIALISABLE with
  type t = A.t * B.t
= struct
...
module MTriple
    (A : SERIALISABLE)
    (B : SERIALISABLE)
    (C : SERIALISABLE) : SERIALISABLE with
  type t = A.t * B.t * C.t
= struct
...
module MQuadruple
    ...
    (B : SERIALISABLE)
    (C : SERIALISABLE)
    (D : SERIALISABLE) : SERIALISABLE with
