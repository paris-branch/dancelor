...
module Ly = struct
  let cache : (Set.t * SetParameters.t, string Lwt.t) Cache.t = Cache.create ()

  let render ?(parameters=SetParameters.none) set =
    Cache.use ~cache ~key:(set, parameters) @@ fun () ->
...
module Pdf = struct
  let cache : (Set.t * SetParameters.t option, string Lwt.t) Cache.t = Cache.create ()
