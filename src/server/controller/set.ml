...
module Ly = struct
  ...
  let render ?(parameters = SetParameters.none) set =
    ...
    StorageCache.use ~cache ~key: (`Ly, set, parameters, body) @@
    fun _hash ->
    let parameters = SetParameters.fill parameters in
    let (res, prom) =
      Format.with_formatter_to_string_gen @@
      fun fmt ->
      ...
      fpf
        fmt
        [%blob "template/set/header.ly"]
        title
        (Kind.Dance.to_string kind)
...
module Pdf = struct
  ...
  let render ?parameters set =
    let%lwt body = Set.lilypond_content_cache_key set in
    StorageCache.use ~cache ~key: (`Pdf, set, parameters, body) @@
    fun hash ->
