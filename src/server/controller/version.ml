...
let prepare_ly_file ?(parameters = VersionParameters.none) ?(show_meta = false) ?(meta_in_title = false) ~fname version =
  ...
  let lilypond =
    Format.with_formatter_to_string
    @@ fun fmt ->
...
module Svg = struct
  ...
  let render ?parameters version =
    let%lwt body = Version.content version in
    StorageCache.use ~cache ~key: (`Svg, version, parameters, body)
    @@ fun hash ->
...
module Pdf = struct
  ...
  let render ?parameters version =
    let%lwt body = Version.content version in
    StorageCache.use ~cache ~key: (`Pdf, version, parameters, body)
    @@ fun hash ->
...
module Ogg = struct
  ...
  let render ?parameters version =
    let%lwt body = Version.content version in
    StorageCache.use ~cache ~key: (`Ogg, version, parameters, body)
    @@ fun hash ->
