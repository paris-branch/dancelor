...
let get id : Model_builder.Core.Version.entry option Lwt.t =
  ...
  lwt @@ Option.map (of_json id) (Version_sql.get db ~id: (Entry.Id.to_string id))

let get_all () =
  Connection.with_ @@ fun db ->
  lwt @@ Version_sql.List.get_all db (fun ~id ~json -> of_json (Entry.Id.of_string_exn id) json)
...
let create version =
  ...
  let%lwt _ =
    Connection.with_ @@ fun db ->
    lwt @@ Version_sql.update db ~id: (Entry.Id.to_string id) ~json
  in
  lwt version
