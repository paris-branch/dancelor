...
let get (slug : Model.Set.t Slug.t) = Unsafe.Set.Static.get slug
let get_all = Unsafe.Set.Static.get_all
let save = Unsafe.Set.Static.save

let get_programs_that_contain (slug : Model.Set.t Slug.t) : Model.Program.t list Lwt.t =
  let%lwt all = Dancelor_server_database_unsafe.Program.Static.get_all () in
