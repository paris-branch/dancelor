      ...
      (
        ...
        let t =
          Create.lambda_sig
            loc
            (types @ [Nolabel, acc 0])
            [%type: ([%t variant_type] -> [%t result_type]) * [%t acc (List.length variants)]
            ]
        in
...
module Gen_str = struct
    ...
    let helpers, variants =
      List.mapi
        ...
        ~f: (fun rank v ->
            ...
            in
            let variant =
              [%stri let [%p pvar ~loc uncapitalized] = {
            ...
            let match_fun ~name ~true_case ~false_expr =
              let cases =
                if multiple_cases then [true_case; case ~guard: None ~lhs: [%pat? _] ~rhs: false_expr]
                else [true_case]
              in
              let body =
                if Variant_constructor.is_gadt v then locally_abstract_match loc cases ~variant_ty
  ...
  let v_fold_fun loc variants =
    ...
    let variant_fold acc_expr variant =
      ...
      [%expr [%e evar ~loc (variant_name ^ "_fun__")] [%e acc_expr] [%e evar ~loc variant_name]
      ]
    in
                ...
                pexp_let
                  ...
                  [
                    value_binding
                      ...
                      ~expr: [%expr [%e evar ~loc (variant_name ^ "_fun__")]
  ...
  let v_to_rank loc variants ~variant_ty =
    ...
    in
    let body =
      if List.exists variants ~f: Variant_constructor.is_gadt then locally_abstract_match loc cases ~variant_ty
      else pexp_function ~loc cases
    in
